...
public class Solution2304 {
    public int minPathCost(int[][] grid, int[][] moveCost) {
        ...
        int m = grid[0].length;
        int [] paths = new int[m];
        int [] nextPaths = new int[m];
        System.arraycopy(grid[0],0,paths,0,m);
        for(int i = 1;i <= n;i++){
            int [] pa = grid[i - 1];
            int [] ca = grid[i];
            for(int j = 0;j < m;j++){
                int c = ca[j];
                nextPaths[j] = 0x7fffffff;
                for(int k = 0,h;k < m;k++){
                    h = paths[k] + moveCost[pa[k]][j] + c;
                    if(h < nextPaths[j]){
                        nextPaths[j] = h;
                    }
                }
